...
const dinorankMiddleware = (prefix) =>
  createProxyMiddleware({
    ...
    onProxyRes: responseInterceptor(
      async (responseBuffer, proxyRes, req, res) => {
        ...
        if (
          req.path == "/" ||
          (!req.user.isAdmin && /\/cierra.php/.test(req.path))
        ) {
        ...
        if (proxyRes.headers["location"]) {
          ...
          res.setHeader(
            "location",
            proxyRes.headers["location"].replace("https://dinorank.com", domain)
          );
        }
        if (
          ...
          links.each(function () {
            ...
            if (href !== undefined) {
              $(this).attr(
                "href",
                href.replace("https://dinorank.com", domain)
              );
            }
          });
          let forms = $("form");
          forms.each(function () {
            ...
            if (action !== undefined) {
              $(this).attr(
                "action",
                action.replace("https://dinorank.com", domain)
              );
            }
          });
          let anchors = $("a");
          ...
          scripts.each(function () {
            ...
            if (src == undefined) {
              ...
              if (
                content.includes(
                  "Es necesario iniciar sesión con usuario y contraseña para poder acceder."
                ) ||
                content.includes("Login with user and password is needed") ||
                content.includes(
                  "Es necesario logarse con usuario y clave antes de entrar"
                ) ||
                content.includes(
                  "È necessario accedere con nome utente e password prima di entrare"
                ) ||
                content.includes(
                  "Sie müssen sich mit Benutzer und Passwort anmelden"
                )
              ) {
                newContent = `
          ...
          if (typeof req.user == "object" && !req.user.isAdmin) {
            if (/\/usuario/.test(req.path)) {
              $(
                ".interiorContenedor ul.tabs li:nth-child(2), .interiorContenedor ul.tabs li:nth-child(3)"
              ).remove();
          ...
          menus.each(function () {
            ...
            if (onclick !== undefined) {
              $(this).attr(
                "onclick",
                onclick.replace("https://dinorank.com", domain)
              );
            }
          });
