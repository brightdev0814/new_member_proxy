...
const majesticSearchHistoryModel = require("../models/majesticSearchHistory");
...
const indexmenowMiddleware = require("./indexmenowMiddleware");
const publicwwwMiddleware = require("./publicwwwMiddleware");
const searchatlasMiddleware = require("./searchatlas/dashboardMiddleware");
const wincherMiddleware = require("./wincherMiddleware");
...
const semrushLimitMiddleware = async (req, res) => {
  if (!req.url.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    ...
    let { wpSite } = req;
    if (!isAdmin && /\/analytics\/overview\//.test(req.originalUrl)) {
      const total = await domainOverviewModel.countRequests(
        id,
        username,
        wpSite,
        "semrush"
      );
      const limit = await settingModel.getOverviewLimit(
    ...
    if (
      ...
      const total = await keywordOverviewModel.countRequests(
        id,
        username,
        wpSite,
        "semrush"
      ...
      typeof req.body == "object" &&
      ((/\/kwogw\/rpc/.test(req.originalUrl) &&
        req.body.method == "fts.GetKeywords") ||
        (/\/kmtgw\/rpc/.test(req.originalUrl) &&
          req.body.method == "fts.GetKeywords")) &&
      req.body.id == 5
    ) {
...
const spyfuLimitMiddleware = async (req, res, next) => {
  if (!req.originalUrl.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    ...
    if (
      !isAdmin &&
      (req.path == "/account" || req.path == "/account/subscription")
    ) {
      return {
        next: false,
        redirect: true,
...
const pipiadsLimitMiddleware = async (req, res, next) => {
  if (!req.originalUrl.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    ...
    if (
      ...
      let total = await pipiadsOverviewModel.countRequests(
        id,
        username,
        wpSite,
        "pipiads",
      ...
      let limit = await settingModel.getOverviewLimit(
...
const keywordkegLimitMiddleware = async (req, res) => {
  if (req.method == "POST" && req.path == "/sf") {
    ...
    if (isAdmin) {
      let wpSite = req.wpSite;
      let total = await keywordOverviewModel.countRequests(
        id,
        username,
        wpSite,
        "keywordkeg"
...
const ranxplorerLimitMiddleware = async (req, res) => {
  if (!req.url.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    let { id, username, isAdmin } = req.user;
    let { wpSite } = req;
    if (!isAdmin && /\/domaine\/synthese/.test(req.originalUrl)) {
      const total = await domainOverviewModel.countRequests(
        id,
        username,
        wpSite,
    ...
    if (!isAdmin && /\/motcle\/supercocon/.test(req.originalUrl)) {
      const total = await keywordOverviewModel.countRequests(
        id,
        username,
        wpSite,
    ...
    if (
      (!isAdmin && /\/domaine\/exportkeywords/.test(req.originalUrl)) ||
      ...
      const total = await exportOverviewModel.countRequests(
        id,
        username,
        wpSite,
        "ranxplorer"
...
const majesticLimitMiddleware = async (req, res, next) => {
  if (!req.url.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    let { id, username, isAdmin } = req.user;
    let { wpSite } = req;
    if (!isAdmin && /\/account\/logout/.test(req.path)) {
      return {
        next: false,
        redirect: true,
        path: "/account#my-campaigns-tab",
      };
    }
    if (
      !isAdmin &&
      /\/reports\/bulk-backlink-checker/.test(req.path) &&
      req.method == "POST"
    ) {
      const total = await domainOverviewModel.countRequests(
        id,
    ...
    if (!isAdmin && /\/reports\/search-explorer/.test(req.path)) {
      const total = await majesticSearchHistoryModel.countSearches(
          id,
          username,
          wpSite
      );
      ...
      if (total >= limit) {
        return {
          next: false,
          data: `You have reached your limit searching per day. you can search ${limit} times. `,
        };
      } else {
        await majesticSearchHistoryModel.create({
          userId: id,
          username: username,
          site: wpSite,
          url: req.url
...
const seobserverLimitMiddleware = async (req, res, next) => {
  if (!req.url.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    ...
    if (
      !isAdmin &&
      /\/keywords\/directsearch/.test(req.originalUrl) &&
      req.method == "POST"
    ) {
      const total = await keywordOverviewModel.countRequests(
        id,
    ...
    if (
      !isAdmin &&
      (/\/sites\/view\//.test(req.originalUrl) ||
        /\/sites\/directsearch/.test(req.originalUrl)) &&
      req.method == "POST"
    ) {
      const total = await domainOverviewModel.countRequests(
        id,
...
const copyscapeLimitMiddleware = async (req, res, next) => {
  if (!req.url.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    ...
    if (
      !isAdmin &&
      /\/probatchedit.php/.test(req.path) &&
      req.method == "POST"
    ) {
      const total = await domainOverviewModel.countRequests(
        id,
  ...
  if (!req.url.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    ...
    if (!isAdmin && /\/ajax\/document\/add/.test(req.path)) {
      ...
      if (req.body.data.obj.data.search.from === "text") {
        total = await domainOverviewModel.countRequests(
          id,
          username,
          wpSite,
          "plagium",
...
const indexmenowLimitMiddleware = async (req, res, next) => {
  if (!req.url.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    let { id, username, isAdmin } = req.user;
    let { wpSite } = req;
    if (!isAdmin && /\/project\/add/.test(req.path) && req.method == "POST") {
      const total = await domainOverviewModel.countRequests(
        id,
        username,
        wpSite,
      ...
      if (total >= limit) {
        return {
          next: false,
          data: `You have reached your limit creating project per day. you can only create ${limit} projects a day.`,
        };
      } else {
        await domainOverviewModel.create({
          userId: id,
...
const publicwwwLimitMiddleware = async (req, res, next) => {
  if (!req.url.match(/\.(css|json|js|text|png|jpg|map|ico|svg)/)) {
    const { id, username, isAdmin } = req.user;
    const { wpSite } = req;
    if (!isAdmin && /^\/$/.test(req.path) && req.query.q) {
      const total = await domainOverviewModel.countRequests(
        id,
        username,
        wpSite,
      ...
      const limit = await settingModel.getOverviewLimit("publicwwwSearchLimit");
      if (total >= limit) {
        return {
          next: false,
          data: `You have reached your search limitation per day. you can only search ${limit} times a day.`,
        };
      } else {
        await domainOverviewModel.create({
          userId: id,
...
const applyMiddleware = async (req, res, next) => {
  ...
  if (proxy !== null) {
    if (setting != null) {
      let prefix =
        typeof req.cookies.prefix == "undefined" || req.cookies.prefix == ""
          ? "www"
          : req.cookies.prefix;
      req.proxy = {
        prefix,
      ...
      if (proxy.type == "semrush") {
        ...
        if (staticProxy) {
          ...
          return res.render("warning", {
            msg: "Please set a domain name for a static server.",
          });
        }
      ...
      } else if (proxy.type === "toolindexmenow") {
        req.proxy.projectLimit = setting["toolindexmenowProjectLimit"];
        req.proxy.linkLimit = setting["toolindexmenowLinkLimit"];
        let result = await indexmenowLimitMiddleware(req, res, next);
        if (result.next) {
          return indexmenowMiddleware(prefix)(req, res);
        } else {
          return res.render("warning", { msg: result.data });
        }
      } else if (proxy.type === "publicwww") {
        let result = await publicwwwLimitMiddleware(req, res, next);
        if (result.next) {
          return publicwwwMiddleware(prefix)(req, res);
        } else {
          return res.render("warning", { msg: result.data });
        }
      } else if (proxy.type === "searchatlas") {
        return searchatlasMiddleware(prefix)(req, res);
      } else if (proxy.type == "wincher") {
        if (req.proxy.cookie) {
          let cookies = req.proxy.cookie.split("; ");
          res.cookie(
            "wincher_access_token",
            cookies[0].replace("wincher_access_token=", ""),
            {
              path: "/",
              domain:
          ...
          res.cookie(
            "wincher_httpreferer",
            cookies[1].replace("wincher_httpreferer=", ""),
            {
              path: "/",
              domain:
                process.env.NODE_ENV === "development" ? undefined : domain,
          ...
          res.cookie(
            "wincher_refresh_token",
            cookies[2].replace("wincher_refresh_token=", ""),
            {
              path: "/",
              domain:
                process.env.NODE_ENV === "development" ? undefined : domain,
        ...
        return wincherMiddleware(prefix)(req, res, next);
      }
    } else {
      return res.render("warning", {
        msg: "Admin have to set up some proxy-related setting.",
      });
    }
  } else {
    return res.render("warning", {
      msg: "The domain is not registered in our application.",
    });
  }
};
