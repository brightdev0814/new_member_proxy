    ...
    onProxyRes: responseInterceptor(
        async (responseBuffer, proxyRes, req, res) => {
            ...
            if (proxyRes.headers["content-type"] && proxyRes.headers["content-type"].includes("text/html")) {
                ...
                $(".header_top .r .select.nice-select option:nth-child(1)").val(domain);
                $(".header_top .r select.nice-select option:not(:first-child)").remove();
                $(".header_top .r div.nice-select li:nth-child(1)").attr("data-value", domain);
                $(".header_top .r div.nice-select li:not(:first-child)").remove();
                if (typeof req.user == "object" && !req.user.isAdmin) {
                    $(".header_top .menu > li").remove();
                    if (req.path == "/profile") {
                        $("table a").remove();    
                    }
                }
                ...
                var content = $.html();
                content = content.replaceAll("https://1.fr", domain);
