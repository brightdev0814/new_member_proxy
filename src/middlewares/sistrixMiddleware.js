...
module.exports = async function (request, reply) {
  ...
  if (typeof request.headers["referer"] !== "undefined") {
    refererUrl = handlerHelpers.getRealReferer(
      targetedHost,
      currentDomain,
  ...
  if (handlerHelpers.urlContainsOriginalHost(request.url)) {
    ...
    if (
      handlerHelpers.urlContainsOriginalHost(request.headers["referer"] + "")
    ) {
      targetedHost = handlerHelpers.extractOriginalHost(
        request.headers["referer"] + ""
      );
    }
  }
  ...
  const serviceDomainRegExp = new RegExp(
    SERVICE_ROOT_DOMAIN.replace(/\./, ".")
  );
  ...
  const allowedRequestHeaders = handlerHelpers.filterRequestHeaders(
    request.headers,
    excludedHeaders,
    someHeadersValue
  );
  ...
  if (/post|put|patch/i.test(request.method)) {
    ...
    if (typeof requestData === "string") {
      ...
      requestData = requestData
        .replace(domainRegExp, targetedHost)
        .replace(
          new RegExp(handlerHelpers.MCOP_LOCATION_STR, "mg"),
          "location"
        );
  ...
  if (typeof portNumber !== "undefined") webClient.setPort(portNumber);
  ...
  if (handlerHelpers.isHtml(serverRes.headers["content-type"])) {
    ...
    if (body.length > 0) {
      ...
      if (
        typeof request.user.isAdmin !== "undefined" &&
        !request.user.isAdmin
      ) {
        if (/^\/account/.test(request.path)) {
          $("#body #content").html(
            "<h1 style='text-align: center'>Access permission is denied.</h1>"
          );
        }
        $(".simple-right").remove();
      }
      ...
      if (request.headers["ajax"] === "1") {
        const clicks = $(".row.preview-primary[onclick]");
    ...
    body = body
      .toString()
      .replace(new RegExp(regx, "g"), `https://${currentDomain}`);
    body = body.replace(
      new RegExp(`https:\/\/${targetedHost}`),
      `https:\/\/${currentDomain}`
    );
    body = Buffer.from(body, "utf-8");
  } else if (
    handlerHelpers.isJsCode(serverRes.headers["content-type"] + "") ||
    /\.js/.test(request.url)
  ) {
...
internals.removeUselessParts = function ($, req, targetedHost) {
  ...
  const dataLinks = $(".logo-dropdown-menu-top .logo-dropdown-item");
  const dataLink2s = $(".logo-dropdown-menu .logo-dropdown-item");
  scripts.each(function () {
    ...
    if (typeof xlinkHref !== "undefined") {
      $(this).attr(
        "xlink:href",
        handlerHelpers.modifyUrl(xlinkHref, currentDomain)
      );
    }
  });
  ...
  dataLinks.each(function () {
    ...
    if ($(this).hasClass("amazon")) $(this).remove();
    if ($(this).hasClass("ai")) $(this).remove();
    $(this).attr(
      "data-link",
      handlerHelpers.modifyUrl(dataLink, currentDomain)
    );
  });

  dataLink2s.each(function () {
    const dataLink = $(this).attr("data-link");
    if ($(this).hasClass("amazon")) $(this).remove();
    if ($(this).hasClass("ai")) $(this).remove();
    $(this).attr(
...
internals.injectScriptInXml = async function (xmlDoc, req) {
  ...
  if (Array.isArray(matchesItems)) {
    for (let i = 0; i < matchesItems.length; i++) {
      let newUrl = handlerHelpers.modifyUrl(
        (matchesItems[i] + "").replace('href="', "").replace('"', ""),
        localHost
      );
      ...
      if (/&/.test(newUrl)) {
        ...
        newUrl =
          ...
          urlObjt.pathname +
          "?" +
          handlerHelpers.MCOP_COMPOSITE_GET_VAR_NAME +
          "=" +
          btoa(urlObjt.search);
      }
...
internals.isForbiddenUrl = function (url) {
  return (
    typeof url !== "string" ||
    url === "/" ||
    /\/account\/(settings|users|modules|invoices|google|labs|learning-hub|shortlinks|pins|watchlist|downloads|alerts|api)/.test(
      url
    ) ||
    /\/sistrix\/logout/.test(url) ||
    /\/sistrix\/upgrade/.test(url) ||
